...
Item {
    ...
    Loader {
        ...
        onLoaded: {
            if (key !== "" && parameter !== "") {
                ...
                item[key + "Changed"].connect(function(){
                    root.isChanged = root.defaultValue != item[key];
                })
            }
        }
    }
    ...
    SPButton {
        ...
        enabled: (root.key !== null && root.defaultValue !== null)
        icon.source: "../icons/icon_cycle.png"
        icon.width: 14
        icon.height: 14
        onClicked: {
            __contentItem.item[root.key] = root.defaultValue
        }
    }
}
